...
class Boggle():
    ...
    def make_board(self, board_size=6):
        """Make and return a random boggle board."""
        self.board_size = board_size
        board = []

        for y in range(self.board_size):
            row = [choice(string.ascii_uppercase) for i in
    ...
    def find_from(self, board, word, y, x, seen):
        ...
        # mean that once we try a letter in one call, it could never be
        # tried again,
        ...
        # point(y,x))."
        #
        # (this could be written with an augmented operator as "seen |=
        # {(y, x)}",
        # in the same way "x = x + 2" can be written as "x += 2", but
        # that would seem
        ...
        if y <= self.board_size and x <= self.board_size:
            if self.find_from(board, word[1:], y + 1, x + 1, seen):
                return True

        if x > 0 and y <= self.board_size:
            if self.find_from(board, word[1:], y + 1, x - 1, seen):
                return True

        if x <= self.board_size and y > 0:
            if self.find_from(board, word[1:], y - 1, x + 1, seen):
                return True
